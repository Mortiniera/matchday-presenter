...
# Sub-agent factories
from reddit.agent import create_agent as create_reddit_scout_agent
...
async def create_coordinator_agent():
    ...
    coordinator = Agent(
        ...
        instruction=(
            "You manage three sub-agents: Reddit Agent, Summarizer, and Speaker."
            "\n1. When the user asks for 'hot posts', delegate to Reddit Agent and return its raw list."
            "\n2. If the user then asks for a 'summary', delegate the Reddit Agent's exact output to Summarizer and return its summary."
